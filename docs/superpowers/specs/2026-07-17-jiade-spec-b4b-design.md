...
1. **loan 服务**（loan_db）：贷款域 6 表 schema（忠实移植 bossy）+ 确定性 fixture（静态一次性 + 月度还款 + 逐日快照滚存 + 逾期滑落）+ 完整四层只读 API + 1 联邦端点。
...
- **bossy 未生成的字段补全** → 除 `wealth_income`（本次按 Q1-B 补每日利息）外，其余忠实 bossy 生成范围。
...
## 5. schema（忠实移植 bossy + 补索引）

### 5.1 loan_db（6 表）

`loan_product` / `loan_account` / `loan_disbursement` / `loan_repay` / `loan_overdue` / `loan_balance`。DDL 原样移植 bossy `loan_db.sql`（列定义零差异），仅**补索引**（`biz_date` / `loan_no` / `cust_id` / `product_code`），对齐 reward/risk DDL 的索引风格。金额字段（`principal`/`balance`/`amount`/`principal_amt`/`interest_amt`/`paid_*`/`overdue_amount`/`principal_balance`/`interest_receivable`/`max_amount`）为 `NUMERIC(18,2)`；`rate`/`min_rate`/`max_rate` 为 `NUMERIC(10,6)`（比率，非金额）。

### 5.2 wealth_db（5 表）

`wealth_product` / `wealth_nav` / `wealth_holding` / `wealth_order` / `wealth_income`。DDL 原样移植 bossy `wealth_db.sql`，补索引。金额字段（`min_amount`/`cost`/`current_value`/`amount`）为 `NUMERIC(18,2)`；`nav`/`accum_nav` 为 `NUMERIC(12,6)`、`share` 为 `NUMERIC(18,4)`、`expected_return` 为 `NUMERIC(10,6)`（均非货币小数，按 NUMERIC 文本直存）。
...
- **RunLoan(ctx, db, cfg, accounts)**（rng `seed+41`，单次——无逐日随机，忠实 bossy）：
  ...
  - 逾期选择：遍历 accounts，`if IntRange(1,12)==1 → overdueStart=RandomDate(start, max(start, addMonths(end,-2)))`（~8%，bossy）。
...
**词库（rng.go 新增）**：`WealthProducts`（移植 bossy 6 产品元组：code/name/type/risk/expected_return/min_amount/term_days）、`OrderTypes=["申购","申购","赎回"]`（2/3 申购）、`IncomeTypes=["利息"]`。
...
### 6.5 Jiade 适配（相对 bossy 的有意偏离，均符合既有约定）
...
| interest/nav 计算 | float | 比率×Money-cents→round 到分 / NAV 6dp float 游走 | rate/nav 是比率非金额，中间 float 可接受（结果落 Money/6dp），忠实 bossy |
...
```go
{Host: "loan_db",   Remote: "cust_db", Tables: []string{"cust_info"}},   // bossy 已有
{Host: "wealth_db", Remote: "cust_db", Tables: []string{"cust_info"}},   // bossy 已有
```
...
- **端口占用**：5432 被 bossy-postgres 占用（别碰，reset 会毁 bossy 数据）；集成测试用临时 pg 5433。
