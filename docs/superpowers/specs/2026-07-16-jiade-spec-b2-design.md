...
- **日期**：2026-07-16
- **状态**：设计待审（待用户复审）
- **范围**：Spec B-2 —— core_db 多日切日引擎（移植 bossy `bizdate` + `distribution`，替换 Spec A 一次性快照 fixture）
...
B-2 移植 bossy 的 `bizdate.py`（逐日推进引擎）+ `distribution.py`（trend/seasonal/cyclical 三因子），把这块换成**逐日滚存引擎**：`[StartBizDate, EndBizDate]` 全量日历日逐日生成 `acct_txn`、写当日全账户 `account_balance` 快照、内存滚余额，末尾切 `sys_param.biz_date`。
...
- **gl_balance 历史日**：不补。bossy 的 bizdate 不写 gl_balance；且历史流水是**单腿**（transaction-log 式，非复式凭证），无法有意义地汇总成平衡总账。`gl_balance` 仍只由 B-3 实时复式记账产生（`/ledger` 历史日返回空，与现状一致）。
- **跳过周末/节假日**：不跳，靠 cyclical/seasonal 因子压量（bossy 模式，保留同环比连续性）。
- **loan/wealth 多日数据**：B-4。
- **复式凭证历史流水**：种子历史是单腿 transaction-log（空 `voucher_no`），与 B-3 实时复式凭证共存（B-3 §5 已预留读接口容错）。
- **账户开闭户动态**：全量账户从 StartBizDate 即活跃（对齐 bossy 注释）；不做"开户晚于 d 则不交易"的精细化。
...
| `holidays` | 移植 bossy HOLIDAYS：国庆 2025-10-01/02/03、元旦 2026-01-01、春节 2026-02-16/17/18、双十一 2025-10-10 / 2025-11-11 |
...
→ 单日结果**只依赖日期本身**，子范围重跑/跨日推进时单日可复现（对齐 bossy `volume_fn` 注释）。
...
```go
// DayState 账户余额的内存滚存态（避免逐笔查库，对齐 bossy DailyState）。
...
每日流程（对齐 bossy `advance_biz_dates` 单日内逻辑）：

1. `n = volumeForDay(cfg, date)`
2. 内容 rng = `NewRNG(cfg.Seed + 200 + ordinal(date))`（**每日独立**，比 bossy 顺序 fk 更鲁棒——任意子范围可复现）
3. `for i in [0, n)`：
...
**金额常量**对齐 bossy 量级（plan 阶段可微调）：

| 项 | bossy（元，float） | Jiade（分，int64） |
|----|--------------------|--------------------|
...
3. 末尾 `UPDATE sys_param SET param_value=$1 WHERE param_key='biz_date'`（=EndBizDate）+ `prev_biz_date`（=次末日）。**只切一次**（bossy 逐日切是冗余，最终结果相同）。

**批量 INSERT 分块**：构造多行 `VALUES ($1,$2,...),(...)`，规避 pg 65535 参数限——`acct_txn` 13 列 ≤5000 行/批；`account_balance` 6 列 ≤10000 行/批。

**逐日幂等**：当日 `DELETE` 后 `INSERT`，同日/子范围重跑安全（对齐 bossy `_insert_txns_and_balance`）。整体重跑由 `--reset` DROP 库保证。
...
| 当日交易量 | `seed + 100 + ordinal` | bossy 原样 |
| 当日流水内容 | `seed + 200 + ordinal` | **每日独立**（bossy 用顺序 fk；独立化使任意子范围可复现）|
...
- **sys_param 只切一次**到 EndBizDate（非逐日切；结果同 bossy）。
- **无账户开闭户动态**：全量账户从 StartBizDate 即活跃。
- **daily_txn 量级沿用 bossy**：dev (500,1250) / full (2000,5000)；dev 总量约 35.7 万 txn + 81.6 万余额行（忠实 bossy，未裁剪）。
