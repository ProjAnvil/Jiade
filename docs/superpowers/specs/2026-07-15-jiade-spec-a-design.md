...
- 多库 + FDW 跨库联邦。
- bossy 的完整多日切日引擎（`bizdate.py`/`distribution.py`）。
- 其他行业模板（零售/物流/医疗…）。
- `--name` 重命名、端口自动分配。
...
├── db/migrations/core_db.sql       # 从 bossy core_db.sql 移植
...
| DB 访问 | database/sql + pgx | 贴合 bossy 的 raw SQL 风格，读起来直白 |
...
## 8. fixture 生成器（bossy → Go 移植）

| bossy (Python) | Jiade (Go) `internal/fixtures/` |
|---|---|
...
**Spec A fixture 范围划线**：静态主数据 + 活/定存账户 + 基线 `account_balance` + 少量近期 `acct_txn`。完整多日切日引擎（bossy 的 `bizdate.py`/`distribution.py`）属 Spec B。
...
- 补齐 bank 其余 6 服务（customer/payment/reward/risk/loan/wealth）及对应 schema（bossy 共 41 表）、fixture、只读 API。
- 多库 + FDW 跨库联邦（对齐 bossy）。
- bossy 完整多日切日引擎移植（`bizdate.py`/`distribution.py`）。
