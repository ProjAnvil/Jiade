...
func TestPickSegmentSkipsDrainingAndTieBreaksBySegStart(t *testing.T) {
	...
	if !ok || got.DCN != "dcn01" {
		t.Fatalf("PickSegment = %v,%v, want dcn01 (DRAINING excluded, tie-break by smaller segStart)", got.DCN, ok)
	}
}
...
func TestNextAccountID(t *testing.T) {
	...
	if id, ok := NextAccountID(seg, 0, false); !ok || id != 1001 {
		t.Fatalf("empty segment should start at 1001 (SegStart is the routing boundary), got %d,%v", id, ok)
	}
