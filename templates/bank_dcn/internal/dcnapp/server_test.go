...
// Intra-unit transfer: GNS locates both parties in the same unit -> local transaction, 200 ok.
...
// Cross-unit transfer: submitted to RMB and COMMITTED -> 200 ok.
...
// Access-layer forwarding: the source account is in another unit -> forward the request
// as-is to its endpoint.
...
// Duplicate account creation: MySQL 1062 -> 200 exists.
func TestCreateAccountDuplicateReturnsExists(t *testing.T) {
	...
	s.handleCreateAccount(w, httptest.NewRequest("POST", "/accounts",
		strings.NewReader(`{"accountId":1001,"name":"charlie","initBalance":"100.00"}`)))
