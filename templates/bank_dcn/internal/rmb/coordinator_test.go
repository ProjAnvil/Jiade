...
// register: when txId already exists, idempotently returns the current status without re-inserting.
...
// advance: all steps DONE → COMMITTED.
...
// advance: one step FAILED → creates COMPENSATE_* steps in reverse order for the original DONE steps and publishes them.
...
// handleReceipt: a late DONE receipt (step already FAILED, transaction already COMPENSATED) → reopens the transaction and re-issues the reverse compensation.
...
// advance: all compensation steps DONE → COMPENSATED terminal state.
