// seed opens accounts through the full GNS flow to inject deterministic test data (simulating production route registration).
// The jiade CLI invokes it hard-coded: go run ./cmd/seed --scale=<dev|full> [--reset]
package main
...
// personName composes a Chinese name from the vocabulary lists.
func personName(r *rand.Rand) string {
	return surnames[r.Intn(len(surnames))] + givenNames[r.Intn(len(givenNames))]
}

// initialBalance fixes the first 2 accounts of each unit at 1000.00 (depended on by verify/README); the rest are random in 100–100000.
func initialBalance(r *rand.Rand, i int) string {
	if i < 2 {
		return "1000.00"
	}
	cents := 10000 + r.Int63n(9990001) // 100.00 ~ 100000.00 (unit: cents)
...
func main() {
	...
	for _, seg := range []int{1000, 2000, 3000} {
		r := rand.New(rand.NewSource(int64(seg))) // deterministic per unit
		for i := 0; i < n; i++ {
			name := personName(r)
			...
			body, _ := json.Marshal(map[string]string{
				"name":        name,
				"initBalance": bal,
				"requestId":   fmt.Sprintf("seed-%s-%d-%d", *scale, seg, i), // idempotency key
			})
...
// resetAll clears business data (keeping route_segment) and flushes the GNS route cache.
func resetAll() {
