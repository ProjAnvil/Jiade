...
func main() {
	...
	handlers := &api.Handlers{
		Svc: service.NewRiskService(repo.NewRiskRepo(db, serviceclient.NewCustomerReader(customerv1.NewCustomerQueryServiceClient(customerConn)))),
	}
	...
	go func() {
		log.Printf("risk HTTP 监听 %s (db=%s)", httpAddr, dbName)
		if err := srv.ListenAndServe(); err != nil && !httpx.IsClosed(err) {
			log.Printf("risk HTTP 服务停止: %v", err)
			stop()
		}
	}()
