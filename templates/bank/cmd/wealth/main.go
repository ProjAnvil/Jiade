// Package main is the wealth read-only API service entrance.
package main
...
func run() error {
	...
	telemetryProvider := initTelemetry(signalCtx, "wealth", getenv("INSTANCE_ID", "wealth-1"))
	defer func() {
		shutdownCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		if err := telemetryProvider.Shutdown(shutdownCtx); err != nil {
			log.Printf("wealth telemetry shutdown: %v", err)
		}
		cancel()
	}()
	...
	handlers := &api.Handlers{
		Svc: service.NewWealthService(repo.NewWealthRepo(db, serviceclient.NewCustomerReader(customerv1.NewCustomerQueryServiceClient(customerConn)))),
	}
	httpAddr := getenv("HTTP_ADDR", ":8080")
	srv := httpx.NewServer(httpx.ServerConfig{
		Service:  "wealth",
		Instance: getenv("INSTANCE_ID", "wealth-1"),
		Addr:     httpAddr,
