...
const fulfillmentConsumer = "fulfillment"

// Consumer applies order lifecycle events through the fulfillment Service and
// the transactional Inbox. Each delivery is settled inside the inbox
// transaction; a duplicate event is a no-op.
...
// NewConsumer binds a fulfillment consumer to its store and service.
...
func (consumer *Consumer) ProcessDelivery(ctx context.Context, delivery amqp.Delivery) error {
	if consumer == nil || consumer.store == nil || consumer.store.pool == nil {
		return errors.New("fulfillment consumer store is unavailable")
	}
...
// orderIDEventPayload mirrors the exact shape order emits in
// internal/order/store.go:insertDerivedEvents so the strict
// DisallowUnknownFields decoder accepts it. order.paid.v1 carries only
// order_id.
type orderIDEventPayload struct {
	OrderID string `json:"order_id"`
}

// orderCancelledPayload mirrors order.cancelled.v1: {order_id, reason}.
...
// fulfillmentKey namespaces the inbound idempotency key by event type so a
// cancel-requested.v1 and order.cancelled.v1 for the same order do not collide
// on the inbox side. The event ID makes the key unique per delivery so a
// replayed delivery that survives inbox GC (very rare) still converges.
func fulfillmentKey(orderID, kind, eventID string) string {
	return kind + ":" + orderID + ":" + eventID
}

func decodeFulfillmentEnvelope(event messaging.Event, destination any) error {
...
func validateFulfillmentSubject(event messaging.Event, orderID string) error {
...
func RunConsumer(
	...
	router confirmedFulfillmentRouter,
	queue string,
	consumer *Consumer,
	routing DeliveryRouting,
) error {
	if channel == nil {
		return errors.New("fulfillment consumer channel is nil")
	}
	if consumer == nil {
		return errors.New("fulfillment consumer is nil")
	}
	if router == nil {
		return errors.New("fulfillment confirmed delivery router is nil")
	}
	if err := channel.Qos(20, 0, false); err != nil {
		return fmt.Errorf("configure fulfillment consumer qos: %w", err)
	}
	deliveries, err := channel.Consume(queue, "", false, false, false, false, nil)
	if err != nil {
		return fmt.Errorf("start fulfillment consumer: %w", err)
	}
	for {
		select {
		case <-ctx.Done():
		...
		case delivery, open := <-deliveries:
			if !open {
				...
				return errors.New("fulfillment consumer deliveries closed")
			}
			routed := delivery
...
type confirmedFulfillmentRouter interface {
...
type retryAcknowledger struct {
	...
	publisher confirmedFulfillmentRouter
...
func (acknowledger *retryAcknowledger) Ack(tag uint64, multiple bool) error {
	if acknowledger.original == nil {
		return errors.New("fulfillment delivery acknowledger is unavailable")
	}
...
func (acknowledger *retryAcknowledger) publishThenAck(tag uint64, exchange, key string) error {
	if acknowledger.publisher == nil || acknowledger.original == nil ||
		exchange == "" || key == "" {
		return errors.New("fulfillment delivery routing is unavailable")
	}
	...
	if err := acknowledger.publisher.Route(ctx, exchange, key, amqp.Publishing{
		...
		return fmt.Errorf("route fulfillment delivery to %s: %w", exchange, err)
	}
	if err := acknowledger.original.Ack(tag, false); err != nil {
		return fmt.Errorf("ack routed fulfillment delivery: %w", err)
	}
	return nil
}
...
func (lifecycle *WorkerLifecycle) Wait(ctx context.Context) error {
	if lifecycle == nil {
		return errors.New("fulfillment worker is unavailable")
	}
	select {
	case <-lifecycle.done:
		lifecycle.mu.Lock()
...
func (lifecycle *WorkerLifecycle) ErrIfStopped() error {
	...
	case <-lifecycle.done:
		...
		return errors.New("fulfillment worker stopped")
	default:
		return nil
	}
}
...
func NewRuntimeReadinessWithDependencies(
	...
	return func(ctx context.Context) error {
		if database == nil {
			return errors.New("fulfillment database readiness is unavailable")
		}
		if err := database(ctx); err != nil {
			return err
		}
		if brokerClosed != nil && brokerClosed() {
			return errors.New("fulfillment broker connection is closed")
		}
		if publisher == nil || !publisher.Available() {
			return errors.New("fulfillment publisher is unavailable")
		}
		for index, dependency := range dependencies {
			if dependency == nil {
				return fmt.Errorf("fulfillment dependency %d readiness is unavailable", index)
			}
			if err := dependency(ctx); err != nil {
				return fmt.Errorf("fulfillment dependency %d is unavailable: %w", index, err)
			}
		}
		if len(workers) == 0 {
			return errors.New("fulfillment workers are unavailable")
		}
		for _, worker := range workers {
			if err := worker.ErrIfStopped(); err != nil {
				return fmt.Errorf("fulfillment worker stopped: %w", err)
			}
		}
		return nil
	}
